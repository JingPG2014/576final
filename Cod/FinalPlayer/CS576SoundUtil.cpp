                               ...
                               DWORD dwNumBuffers )
{
	...
	if(wcsstr(strFileName, TEXT(".pcm")))
	{
		...
		strcpy(pPCMFile->m_strFileName, _strFileName);
		...
		m_dwSize = pPCMFile->GetSize();
		m_dwTotalSeconds = pPCMFile->GetLen();
...
//-----------------------------------------------------------------------------
// Name: CSound::SetPosition()
// Desc: Set the play position of the buffer
//-----------------------------------------------------------------------------
HRESULT CSound::SetPosition(DWORD pos)
{
	if( m_apDSBuffer == NULL )
        return CO_E_NOTINITIALIZED;
    ...
    for( DWORD i=0; i<m_dwNumBuffers; i++ )
        hr |= m_apDSBuffer[i]->SetCurrentPosition( pos );
...
//-----------------------------------------------------------------------------
// Name: CSound::GetLen()
// Desc: Gets the total seconds
//-----------------------------------------------------------------------------
DWORD CPCMFile::GetLen()
{
	return m_dwTotalSeconds;
}